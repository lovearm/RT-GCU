...
*                     (e) Scan for Wireless networks available
*                     (f) Analyze scan results
*                     (g) Join a Wireless network
*
...
*                                            INCLUDE FILES
*
* Note(s) : (1) The device configuration template file should be copied to your application folder and modified
*               to follow your requirements. Refer to the User's Manual for more information about how to configure
*               your device.
*
*               We recommend starting with a working configuration from an example project for your wireless module.
*               Micrium might have some projects available only for internal usage, so if no working projects are found
*               online, please ask at support@micrium.com for a configuration file example.
*
*           (2) Most of the time Micrium provides a Wireless device driver which can be found under the
*               following folder:
*
*                   $/Micrium/Software/uC-TCPIP/Dev/WiFi/<module>/net_dev_<module>.h
*
*               If Micrium does not support your network wireless device driver, you will have to write your own device
*               driver starting from the Wireless Device driver template. Before starting to write your own driver, make
*               sure that the driver is not already available.
*
*           (3) The board support package (BSP) template file should be copied to your application folder and modified
*               for your specific board. Refer to the User's Manual for more information about how to write a BSP.
*
*               However, we recommend starting with a working configuration from an example project for your Wireless
*               module. Micrium might have some projects available only for internal usage, so if no working project are
*               found online, please ask at support@micrium.com for a BSP file example specific for your MCU and your
*               Wireless Module.
*********************************************************************************************************
*********************************************************************************************************
*/
...
#include  <BSP/Template/net_bsp_wifi.h>                     /* TODO BSP header.                  See Note #3.           */
...
*                   (b) Initialize a Wireless Interface
*                   (c) Start the Wireless Interface
*                   (d) Configure IP addresses of the Interface
*                   (e) Scan for Wireless network available
*                   (f) Analyze scan results
*                   (g) Join a Wireless network
*
* Argument(s) : none.
...
*
*                   (f) The fifth parameter is the address of the WiFi Manager API function. This API is provided by Micrium and
*                       it's located in:
*
*                           $/uC-TCPIP/Dev/WiFi/Manager/Generic/net_wifi_mgr.h
*
*                   (g) The sixth and last parameter is the address of the WiFi manager configuration data structure. Actually there
*                       are no configuration require with the generic Wifi manager. So this parameters can be kept as null.
*
...
*                       type of the access point is open, the PSK can set to null.
*********************************************************************************************************
*/
...
CPU_BOOLEAN  AppInit_TCPIP_WiFi (void)
{
    ...
    if (err_net != NET_ERR_NONE) {
        return (DEF_FAIL);
                                                                ...
                                                                /* See Note #3.                                         */
    if_nbr = NetIF_Add((void    *)&NetIF_API_WiFi,              /* See Note #3b.                                        */
                       (void    *)&NetDev_API_TemplateWiFiSpi,  /* TODO BSP API,              See Note #3d.             */
                       (void    *)&NetDev_BSP_WiFi,             /* TODO Device configuration, See Note #3e.             */
                       (void    *)&NetDev_Cfg_WiFi_1,           /* TODO PHY driver API,       See Note #3f.             */
                       (void    *)&NetWiFiMgr_API_Generic,      /* TODO PHY configuration,    See Note #3g.             */
                                   DEF_NULL,
                                  &err_net);
    ...
    ctn = NetIF_WiFi_Scan(if_nbr,                               /* See Note #8.                                         */
    ...
    NetIF_WiFi_Join(if_nbr,                                     /* See Note #9.                                         */
...
#endif  /* NET_IPv6_ADDR_AUTO_CFG_MODULE_EN */

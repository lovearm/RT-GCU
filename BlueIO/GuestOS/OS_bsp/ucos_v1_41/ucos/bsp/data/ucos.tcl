...
proc generate {os_handle} {
    ...
    switch $proctype {
        ...
        "psu_cortexa53" {
            ucos_log_put "INFO" "Setting up build for psu_cortexa58."
...
proc ucos_handle_makefile_inc {os_handle} {
    ...
    if {$proctype == "ps7_cortexa9"} {
        ...
        puts $file_handle "override ARCHIVER = arm-xilinx-eabi-gcc-ar"
    }
...
proc ucos_handle_startup {os_handle} {
    ...
    set amp_master [get_property CONFIG.UCOS_AMP_MASTER $os_handle]
    ...
    if {$amp_master == true} {
        puts $config_file "#define UCOS_AMP_MASTER DEF_ENABLED"
    } else {
        puts $config_file "#define UCOS_AMP_MASTER DEF_DISABLED"
    }
...
proc handle_ts {os_handle} {
    ...
    if {[string compare -nocase "none" $ts_timer] != 0 && [llength $ts_timer] != 0} {
        ...
        switch $ts_driver {
            "ucos_axitimer" { 
                ucos_log_put "INFO" "handle_systick: System tick source set to UCOS_TS_AXITIMER."
            ...
            "default" {
                ucos_log_put "INFO" "handle_systick: System tick source set to UCOS_TS_NONE."
...
proc ucos_handle_stdout {os_handle} {
   set stdout [get_property CONFIG.STDIN_OUT $os_handle]
   set sw_proc_handle [get_sw_processor]
   set hw_proc_handle [get_cells [get_property hw_instance $sw_proc_handle]]
   set processor [get_property NAME $hw_proc_handle]

   if {[llength $stdout] == 1 && [string compare -nocase "none" $stdout] != 0} {
        ucos_log_put "INFO" "handle_stdout: Configuring STDOUT peripheral $stdout."
        ...
        set interface_handle [get_sw_interfaces -of_objects $stdout_drv_handle -filter "NAME==ucos_stdout"]
        ...
        switch $uart_driver {
            "ucos_uartps" { 
                ucos_log_put "INFO" "ucos_handle_stdout: STDOUT source set to UCOS_UART_PS7_UART."
                puts $config_file "#define UCOS_STDOUT_DRIVER UCOS_UART_PS7_UART"
                puts $config_file "#define UCOS_STDOUT_DEVICE_ID $device_id"
                set stdout_config true
            }
            
            "ucos_axiuartlite" { 
                ucos_log_put "INFO" "ucos_handle_stdout: STDOUT source set to UCOS_UART_AXI_UART_LITE."
            ...
            "default" {
                ucos_log_put "INFO" "ucos_handle_stdout: STDOUT source set to UCOS_UART_NONE."
                puts $config_file "#define UCOS_STDOUT_DRIVER UCOS_UART_NONE"
                puts $config_file "#define UCOS_STDOUT_DEVICE_ID 0"
                set stdout_config false
            }
        }
        
        #Define STDOUT_BASEADDRESS for compatibility with standalone routines

        if {$stdout_config == true} {
            puts $config_file "#define STDOUT_BASEADDRESS"
        }
   } else {
        ucos_log_put "INFO" "ucos_handle_stdout: No STDOUT specified."
   }

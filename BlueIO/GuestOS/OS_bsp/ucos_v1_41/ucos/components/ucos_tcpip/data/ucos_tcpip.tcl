...
proc generate {ucos_handle} {
    ...
    file mkdir "../../include/Secure/Mocana"
    ...
    set_define "./src/net_cfg.h" "NET_STAT_POOL_BUF_EN"         [expr ([get_property CONFIG.NET_STAT_POOL_BUF_EN               $ucos_handle] == true)?"DEF_ENABLED":"DEF_DISABLED"]
    ...
    set file_handle [open "./src/net_cfg.h" a]
    if {$ipv4_rx_off == "true" || ($ipv4_rx_off == "auto" && $ipv4_rx_off_auto == "true")} {
        puts $file_handle "#define  NET_IPV4_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_IPV4_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_DISABLED"
    }
    
    if {$ipv4_tx_off == "true" || ($ipv4_tx_off == "auto" && $ipv4_tx_off_auto == "true")} {
        puts $file_handle "#define  NET_IPV4_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_IPV4_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_DISABLED"
    }
    
    if {$icmp_rx_off == "true" || ($icmp_rx_off == "auto" && $icmp_rx_off_auto == "true")} {
        puts $file_handle "#define  NET_ICMP_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_ICMP_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_DISABLED"
    }
    
    if {$icmp_tx_off == "true" || ($icmp_tx_off == "auto" && $icmp_tx_off_auto == "true")} {
        puts $file_handle "#define  NET_ICMP_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_ICMP_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_DISABLED"
    }
    
    if {$udp_rx_off == "true" || ($udp_rx_off == "auto" && $udp_rx_off_auto == "true")} {
        puts $file_handle "#define  NET_UDP_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_UDP_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_DISABLED"
    }
    
    if {$udp_tx_off == "true" || ($udp_tx_off == "auto" && $udp_tx_off_auto == "true")} {
        puts $file_handle "#define  NET_UDP_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_UDP_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_DISABLED"
    }
    
    if {$tcp_rx_off == "true" || ($tcp_rx_off == "auto" && $tcp_rx_off_auto == "true")} {
        puts $file_handle "#define  NET_TCP_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_TCP_CFG_CHK_SUM_OFFLOAD_RX_EN  DEF_DISABLED"
    }
    
    if {$tcp_tx_off == "true" || ($tcp_tx_off == "auto" && $tcp_tx_off_auto == "true")} {
        puts $file_handle "#define  NET_TCP_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_ENABLED"
    } else {
        puts $file_handle "#define  NET_TCP_CFG_CHK_SUM_OFFLOAD_TX_EN  DEF_DISABLED"
    }
